...
int SendMsg(SOCKET sock, char* msg, WORD type)
{
	...
	packet.ph.type = type;
	memcpy(packet.msg, msg, strlen(msg)); // msg -> packet(msg)
	...
	do {
		...
		if (iSendByte == SOCKET_ERROR) // 예외처리
		{
	...
	while (1)
	{
		...
		if (userlist.size() > 0)
		{
			...
			for (iter = userlist.begin(); iter != userlist.end(); )
			{
				...
				// 패킷 완성		
				std::list<KUser>::iterator iterSend;
