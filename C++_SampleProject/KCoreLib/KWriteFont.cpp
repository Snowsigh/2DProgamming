...
void KWriteFont::DeleteDeviceResize()
{
	if (m_pd2dColorBrush)m_pd2dColorBrush->Release();
	if (m_pd2dRT)m_pd2dRT->Release();
}

bool KWriteFont::SetRenderTarget(IDXGISurface1* pSurface)
{
...
void KWriteFont::Draw(std::wstring msg, RECT rt, D2D1::ColorF color, IDWriteTextFormat* tf)
{
	m_pd2dRT->BeginDraw();
	D2D_RECT_F fRT;
	fRT.top = rt.top;
...
bool KWriteFont::Frame()
{
	return false;
}

bool KWriteFont::Render()
{
	m_pd2dRT->BeginDraw();
