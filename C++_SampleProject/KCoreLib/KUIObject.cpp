...
#include "KWorld.h"
#include <string>
bool KUIObject::Frame()
{
	KObject2D::Frame();
	return true;
}

bool KUIObject::Render()
{
	KObject2D::Render();
    return false;
}

bool KUIObject::SetVertexData()
{
	//ConvertIndex(m_vPos, m_fWidth, m_fHeight, m_VertexList);
	 ...
	 // d lb           rb 
	 // 12    13    14    15 
	KVector2 vLT = { (float)m_rtOffset.left,  (float)m_rtOffset.top };
	KVector2 vRT = { (float)m_rtOffset.right, (float)m_rtOffset.top };
	KVector2 vLB = { (float)m_rtOffset.left, (float)m_rtOffset.bottom };
	KVector2 vRB = { (float)m_rtOffset.right, (float)m_rtOffset.bottom };
	...
	KVector2 tLT = { vLT.x / fWidth, vLT.y / fHeight };
	KVector2 tRT = { vRT.x / fWidth, vRT.y / fHeight };
	KVector2 tLB = { vLB.x / fWidth, vLB.y / fHeight };
	KVector2 tRB = { vRT.x / fWidth, vRT.y / fHeight };
...
bool KImageObject::Init()
{
	m_bFadeIn = true;
	return true;
}

bool KImageObject::Frame()
{
	m_vColor.x = m_fAlpha;
	m_vColor.y = m_fAlpha;
	m_vColor.z = m_fAlpha;
...
bool KImageObject::Render()
{
	KObject2D::Render();
	return true;
}

void KButtonObject::HitSelect(KBaseObject* pObj, DWORD dwState)
{
	std::string state;
...
bool KButtonObject::Init()
{

    return false;
}

bool KButtonObject::Frame()
{
	KObject2D::Frame();
    return false;
}

bool KButtonObject::Render()
{
	KObject2D::Render();
    return false;
}
